...
$databases['default']['default'] = [
  'database' => getenv('DEFAULT_DATABASE_NAME'),
  'username' => getenv('DEFAULT_DATABASE_USERNAME'),
  'password' => getenv('DEFAULT_DATABASE_PASSWORD'),
  'prefix' => '',
  'host' => getenv('DEFAULT_DATABASE_HOSTNAME'),
  'port' => getenv('DEFAULT_DATABASE_PORT'),
...
$settings['file_temp_path'] = getenv('DEFAULT_SITE_TMP_PATH');
$settings['hash_salt'] = getenv('DEFAULT_SITE_HASH_SALT');
$settings['config_sync_directory'] = getenv('DEFAULT_SITE_FILE_SYNC_PATH');

// Define localhost site ID and URL array.
$localhost = [
  'COVID19_STATISTICS_SITE_ID' => 'COVID19_STATISTICS_SITE_URL',
];

foreach($localhost as $site_id => $site_url) {
  if ($server_name == getenv($site_url)) {
    $app_id = getenv($site_id);
  }
}

// Define variables as possible.
$app_path = $docroot . '/sites/' . $app_id;
$debug = (bool) getenv('DEFAULT_DEV_DEBUG') == 'TRUE';
$site_default_path = $app_root . '/' . $site_path;

// Include default settings and services files.
if (file_exists($app_path . '/settings.php')) {
  include $app_path . '/settings.php';
}
if (file_exists($site_default_path . '/settings.local.php')) {
  include $site_default_path . '/settings.local.php';
}

// Enable debugger.
if ($debug && file_exists($site_default_path . '/services.local.yml')) {
  $settings['cache']['bins']['render'] = 'cache.backend.null';
  $settings['cache']['bins']['page'] = 'cache.backend.null';
  $settings['cache']['bins']['dynamic_page_cache'] = 'cache.backend.null';
  $settings['container_yamls'][] = $site_default_path . '/services.local.yml';
}

// Overrides the default settings and services files per site.
if (file_exists($app_path . '/settings.local.php')) {
  include $app_path . '/settings.local.php';
}
if (file_exists($app_path . '/services.local.yml')) {
  $settings['container_yamls'][] = $app_path . '/services.local.yml';
}
